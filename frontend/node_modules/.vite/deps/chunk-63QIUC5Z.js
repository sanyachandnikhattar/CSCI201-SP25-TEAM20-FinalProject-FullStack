import {
  ...
  useFormControl,
  useId_default
} from "./chunk-LMJTDGGU.js";
import {
  LocalizationProvider,
  MuiPickersAdapterContext,
  useThemeProps
} from "./chunk-PXDEILGQ.js";
import {
  HTMLElementType,
...
var useFieldState = (parameters) => {
  ...
  const valueRef = React56.useRef(value);
  React56.useEffect(() => {
  ...
  const error = React56.useMemo(() => {
    if (errorProp !== void 0) {
      return errorProp;
    }
    return hasValidationError;
  }, [hasValidationError, errorProp]);
  const localizedDigits = React56.useMemo(() => getLocalizedDigits(utils), [utils]);
  const sectionsValueBoundaries = React56.useMemo(() => getSectionsBoundaries(utils, localizedDigits, timezone), [utils, localizedDigits, timezone]);
  const getSectionsFromValue = React56.useCallback((valueToAnalyze) => fieldValueManager.getSectionsFromValue(valueToAnalyze, (date) => buildSectionsFromFormat({
    utils,
  ...
  const parsedSelectedSections = React56.useMemo(() => parseSelectedSections(selectedSections, state.sections), [selectedSections, state.sections]);
  const activeSectionIndex = parsedSelectedSections === "all" ? 0 : parsedSelectedSections;
  const sectionOrder = React56.useMemo(() => getSectionOrder(state.sections, isRtl && !enableAccessibleFieldDOMStructure), [state.sections, isRtl, enableAccessibleFieldDOMStructure]);
  const areAllSectionsEmpty = React56.useMemo(() => state.sections.every((section) => section.value === ""), [state.sections]);
  ...
  const sectionToUpdateOnNextInvalidDateRef = React56.useRef(null);
...
function useFieldInternalPropsWithDefaults(parameters) {
  ...
  const handleChangeFromPicker = React57.useCallback((newValue, ctx) => {
    return setValue == null ? void 0 : setValue(newValue, {
      validationError: ctx.validationError,
      shouldClose: false
    });
  }, [setValue]);
  const internalPropsWithDefaultsFromContext = React57.useMemo(() => {
...
function useFieldHiddenInputProps(parameters) {
  ...
  const valueStr = React58.useMemo(() => areAllSectionsEmpty ? "" : fieldValueManager.getV7HiddenInputValueFromSections(state.sections), [areAllSectionsEmpty, state.sections, fieldValueManager]);
  return {
    value: valueStr,
...
function useFieldSectionContainerProps(parameters) {
  ...
  return React59.useCallback((sectionIndex) => ({
...
function useFieldSectionContentProps(parameters) {
  ...
  return React60.useCallback((section, sectionIndex) => {
    const sectionBoundaries = sectionsValueBoundaries[section.type]({
...
var useFieldV7TextField = (parameters) => {
  ...
  const domGetters = React61.useMemo(() => ({
  ...
  React61.useImperativeHandle(unstableFieldRef, () => ({
...
function useApplyDefaultValuesToDateFieldInternalProps(internalProps) {
  ...
  return React63.useMemo(() => _extends({}, internalProps, validationProps, {
...
function useMeridiemMode(date, ampm, onChange, selectionState) {
  const utils = useUtils();
  const cleanDate = React65.useMemo(() => !utils.isValid(date) ? null : date, [utils, date]);
  const meridiemMode = getMeridiem(cleanDate, utils);
  const handleMeridiemChange = React65.useCallback((mode) => {
...
var PickersArrowSwitcher = React66.forwardRef(function PickersArrowSwitcher2(inProps, ref) {
  const isRtl = useRtl();
  const props = useThemeProps({
    props: inProps,
    name: "MuiPickersArrowSwitcher"
...
//# sourceMappingURL=chunk-63QIUC5Z.js.map
